...
IAOAPDevice::Pointer AOAPDevice::create(IUSBWrapper& usbWrapper, asio::io_service& ioService, DeviceHandle handle)
{
    ...
    const auto *interface = AOAPDevice::getInterface(configDescriptorHandle);
    const auto *interfaceDescriptor = AOAPDevice::getInterfaceDescriptor(interface);
...
ConfigDescriptorHandle AOAPDevice::getConfigDescriptor(IUSBWrapper& usbWrapper, const DeviceHandle& handle)
{
