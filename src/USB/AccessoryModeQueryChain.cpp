...
{
    auto queryPromise = IAccessoryModeQuery::Promise::defer(strand_);
    queryPromise->then([this, self = this->shared_from_this()](const IUSBEndpoint::Pointer& usbEndpoint) mutable {
            this->startQueryHandler(usbEndpoint);
        },
        [this, self = this->shared_from_this()](const error::Error& e) mutable {
            promise_->reject(e);
            promise_.reset();
...
void AccessoryModeQueryChain::startQueryHandler(const IUSBEndpoint::Pointer& usbEndpoint)
{
    activeQuery_.reset();
