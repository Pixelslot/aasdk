...
void USBHub::cancel()
{
    strand_.dispatch([this, self = this->shared_from_this()]() mutable {
        ...
        std::for_each(queryChainQueue_.begin(), queryChainQueue_.end(), [](auto && PH1) { PH1->cancel(); });
...
int USBHub::hotplugEventsHandler(libusb_context* usbContext, libusb_device* device, libusb_hotplug_event event, void* userData)
{
    if(event == LIBUSB_HOTPLUG_EVENT_DEVICE_ARRIVED)
    {
        ...
        self->strand_.dispatch([self, device] { self->handleDevice(device); });
    }
...
void USBHub::handleDevice(libusb_device* device)
{
    ...
    else
    {
        ...
        queryChainPromise->then([this, self = this->shared_from_this(), queueElementIter](const DeviceHandle& handle) mutable {
