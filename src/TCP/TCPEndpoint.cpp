...
{
    tcpWrapper_.asyncWrite(*socket_, buffer,
                           [this, self = this->shared_from_this(), promise](const asio::error_code& ec, size_t bytesTransferred){asyncOperationHandler(ec, bytesTransferred, promise);});
}

void TCPEndpoint::receive(common::DataBuffer buffer, Promise::Pointer promise)
{
    tcpWrapper_.asyncRead(*socket_, buffer,
                          [this, self = this->shared_from_this(), promise](const asio::error_code& ec, size_t bytesTransferred){asyncOperationHandler(ec, bytesTransferred, promise);});
}
...
void TCPEndpoint::asyncOperationHandler(const asio::error_code& ec, size_t bytesTransferred, const Promise::Pointer& promise)
{
    if(!ec)
    {
